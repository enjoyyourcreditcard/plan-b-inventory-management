    ...
    <div class="grid grid-cols-12 gap-6 mb-14">
        <div class="col-span-12 lg:col-span-12 mt-6">
            <div class="box p-16 overflow-hidden bg-primary intro-y">
                <div class=" w-full  text-white text-3xl -mt-3">Selamat {{ Carbon\Carbon::now()->format('H') <= 10 ? "Pagi" : ( Carbon\Carbon::now()->format('H') <= 14 ? "Siang" : ( Carbon\Carbon::now()->format('H') <= 18 ? "Sore" : "Malam" ) ) }} {{ Auth::user()->name }} !
                </div>
                                ...
                                <div class="box px-4 py-4 mb-3 flex items-center zoom-in">
                                    <div class="w-10 h-10 flex-none image-fit rounded-md overflow-hidden">
                                        <img alt="Midone - HTML Admin Template"
                                            src="http://tinker.left4code.com/dist/images/profile-5.jpg">
                                    </div>
                                    <div class="ml-4 mr-auto">
                                        <div class="font-medium">{{ $data->grf_code }}</div>
                                        <div class="text-slate-500 text-xs mt-0.5">{{ $data->user->name }}</div>
                                    </div>
                                    <div
                            ...
                            @foreach ($whreturn as $item)
                                <div class="box px-4 py-4 mb-3 flex items-center zoom-in">
                                    ...
                                    <div class="ml-4 mr-auto">
                                        ...
                                        <div class="text-slate-500 text-xs mt-0.5">{{ $item->user->name }}</div>
                                    </div>
                                    <div
                                        class="py-1 px-2 rounded-full text-xs bg-success text-white cursor-pointer font-medium">
                                        {{ $item->status }}</div>
