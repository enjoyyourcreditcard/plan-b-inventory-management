            ...
            <div class="modal-content">
                <div class="modal-body p-0">
                    <div class="p-5 text-center text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-upc-scan w-16 h-16 mx-auto mt-3" viewBox="0 0 16 16">
...
* --}}
    <div id="modal-pieces" class="modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form id="form-pieces" action="{{ Route('warehouse.transfer.post.pieces', $irf->id) }}" method="POST">
                    @csrf
...
* --}}
    <div id="modal-bulk" class="modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-body p-10 text-center">
                    <form id="form-bulk" data-single="true" action="{{ Route('warehouse.transfer.post.bulk', $irf->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <select name="warehouse_id" data-placeholder="Select warehouse" class="tom-select w-full"
                        ...
                        @foreach ($warehouses as $warehouse)
                            <option value="{{ $warehouse->id }}" {{ $warehouse->id == $irf->warehouse_id ? 'selected' : '' }}> {{ $warehouse->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary w-20" data-tw-toggle="modal" data-tw-target="#modal-change-current-warehouse-confirmation">Save</button>
                </div>
                <!-- END: Modal Footer -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <select name="warehouse_destination" data-placeholder="Select warehouse" class="tom-select w-full" form="form-change-warehouse-destination" required>
                        @foreach ($warehouses->where('id', '!=', $irf->warehouse_id) as $warehouse)
                            <option value="{{ $warehouse->name }}" {{ $warehouse->name == $irf->warehouse_destination ? 'selected' : '' }}>{{ $warehouse->name }}</option>
                        @endforeach
                    </select>
                </div>
                <!-- END: Modal Body -->
                ...
                <!-- BEGIN: Modal Footer -->
                <form id="form-change-warehouse-destination" action="{{ Route('warehouse.transfer.put.destination', $irf->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                </form>
                <div class="modal-footer">
            ...
            <div class="modal-content">
                <div class="modal-body p-0">
                    <div class="p-5 text-center">
                        ...
                        <div class="text-slate-500 mt-2">All your item on the list will be deleted. <br>This process cannot be undone.</div>
                    </div>
                    <form id="form-change-current-warehouse" action="{{ Route('warehouse.transfer.put.current', $irf->id) }}" method="POST" class="px-5 pb-8 text-center">
                        @csrf
                        @method('PUT')
                    </form>
                    <div class="px-5 pb-8 text-center">
                        <a data-tw-dismiss="modal" class="btn btn-outline-secondary w-24 mr-1">Cancel</a>
                        <button type="submit" class="btn text-white !bg-emerald-700 impor w-24" form="form-change-current-warehouse">Sure</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                <div class="modal-body p-0">
                    <div class="p-5 text-center">
                        ...
                        <div class="text-3xl mt-5">Are you sure?</div>
                        <div class="text-slate-500 mt-2">Are you sure want to submit this transfer? <br>This process cannot be undone.</div>
                    </div>
                    <form action="{{ Route('warehouse.transfer.put') }}" method="POST" class="px-5 pb-8 text-center">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="irf_id" value="{{ isset($transferForms->first()->irf) ? $transferForms->first()->irf_id : null }}">
                        <input type="hidden" name="warehouse_destination" value="{{ isset($transferForms->first()->irf) ? $transferForms->first()->irf->warehouse_destination : null }}">
                        <a data-tw-dismiss="modal" class="btn btn-outline-secondary w-24 mr-1">Cancel</a>
        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="box p-5 rounded-md">
                ...
                <div class="flex items-center">
                    ...
                    </svg> IRF code: <span class="underline decoration-dotted ml-1">{{ $irf->irf_code }}</span>
                </div>

                @if ($irf->warehouse_id)
                    <div class="flex items-center mt-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-building-warehouse w-4 h-4 text-slate-500 mr-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                            <path d="M3 21v-13l9 -4l9 4v13"></path>
                            <path d="M13 13h4v8h-10v-6h6"></path>
                            <path d="M13 21v-9a1 1 0 0 0 -1 -1h-2a1 1 0 0 0 -1 1v3"></path>
                        </svg> Current Warehouse: <span class="underline decoration-dotted ml-1">{{ $irf->warehouse->name }}</span>

                        @if ($irf->status === 'draft')
                            <button data-tw-toggle="modal" data-tw-target="#modal-change-current-warehouse" class="flex items-center ml-auto text-slate-500 transition duration-200 ease-in-out hover:text-slate-300 active:text-slate-700">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square w-4 h-4 mr-2" viewBox="0 0 16 16">
                ...
                @if ($irf->warehouse_id)
                    <div class="flex items-center mt-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-geo-fill w-4 h-4 text-slate-500 mr-2" viewBox="0 0 16 16">
                        ...
                        </svg> Destination: <span class="underline decoration-dotted ml-1">{{ $irf->warehouse_destination }}</span>

                        @if ($irf->status === 'draft')
                            <button data-tw-toggle="modal" data-tw-target="#modal-change-warehouse-destination" class="flex items-center ml-auto text-slate-500 transition duration-200 ease-in-out hover:text-slate-300 active:text-slate-700">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square w-4 h-4 mr-2" viewBox="0 0 16 16">
                                    <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                    <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                </svg>
            ...
            @if ($irf->status == 'draft')
                <div class="intro-y box p-5 rounded-md mt-4">
                    ...
                    <div>
                        <div class="preview">
                            <form action="{{ Route('warehouse.transfer.post.detail', $irf->id) }}" method="POST">
                                @csrf

                                @if ($irf->warehouse_id === null)
                                    <div>
                                ...
                                <div class="mt-3">
                                    ...
                                    <select name="part_id" data-placeholder="Select segment" class="tom-select w-full" required>
                                        <option></option>
                        ...
                        <tbody>
                            ...
                            @forelse ($transferForms as $transferForm)
                                <tr class="intro-x">
                                    ...
                                    <td class="text-center">{{ $transferForm->quantity }}</td>
