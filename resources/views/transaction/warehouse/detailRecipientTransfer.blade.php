...
@section('content')
    <div class="intro-y chat grid grid-cols-12 gap-5 mt-5">
                    ...
                    <div class="pr-1">
                        <div class="box px-5 pt-5 pb-1 mt-5">
                            ...
                            <div class="flex mb-5">
                                ...
                                <p class="text-sm ml-2">
                                    <strong>Current Warehouse :</strong> {{ $irf->warehouse->name }}
                                </p>
                            </div>
                            <div class="flex mb-5">
                                ...
                                <p class="text-sm ml-2">
                                    <strong>Destination :</strong> {{ $irf->warehouse_destination }}
                                </p>
                            </div>
                            <div id="button-submit-recipient" class="flex mb-5 mt-8 ">
                                <form id="form-submit-recipient" action="{{ route('warehouse.post.changeRecipient', $irf->id) }}" method="POST" class="w-full">
                                    @csrf
                                    <button type="submit" class="btn btn-primary w-full rounded-full">
                                        Submit
                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-send mx-2" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
        ...
        <div id="recipient" class="table-responsive overflow-auto col-span-8 lg:col-span-8 2xl:col-span-8" data-grfcode="{{ str_replace( '/', '~', strtolower( $irf->irf_code ) ) }}"></div>
        
        @foreach ($irf->transferForms as $item)
        @endforeach
        ...
        <div id="modal-pieces" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form id="form-pieces" action="{{ route('warehouse.post.storemanual', $item->id) }}" method="POST">
                        @csrf
        ...
        <div id="modal-bulk" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-body p-10 text-center">
                        <form id="form-bulk" data-single="true" action="{{ route('warehouse.post.bulkRecipient', $item->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
        ...
        <div id="modal-non-sn" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form id="form-non-sn" action="{{ route('warehouse.post.non-sn', $item->id) }}"
                        method="POST">
                        @csrf
                        <div class="modal-body p-10 text-center">
