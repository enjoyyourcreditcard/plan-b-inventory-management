...
class BuildService
{
    ...
    public function handleAllBuildApi()
    {
        $buildArray = collect();
        $buildUnique = $this->build->select('name')->distinct()->get();
        foreach ($buildUnique as $item) {
            $build = $this->build->where('name', $item->name)->with("part")->get();
            $buildCollect = collect(["name"=>$item->name, "part"=>$build]);
            $buildArray->push($buildCollect);
        }
        return ($buildArray);
    }
